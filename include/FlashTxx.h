...
#define RAMFUNC __attribute__ ((section(".fastrun"), noinline, noclone, optimize("Os"), used ))
...
extern RAMFUNC void eepromemu_flash_write(void *addr, const void *data, uint32_t len);
extern RAMFUNC void eepromemu_flash_erase_sector(void *addr);
extern RAMFUNC void eepromemu_flash_erase_32K_block(void *addr);
extern RAMFUNC void eepromemu_flash_erase_64K_block(void *addr);
...
RAMFUNC int  flash_write_block( uint32_t addr, char *data, uint32_t count );
RAMFUNC int  flash_erase_block( uint32_t address, uint32_t size );
